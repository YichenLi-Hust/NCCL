...
def resnet5(class_num, pretrained=False, path=None, **kwargs):
    """
    ...
    model = ResNet(Bottleneck, [1, 0, 0], class_num, **kwargs)
    if pretrained:
...
def resnet11(class_num, pretrained=False, path=None, **kwargs):
    """
    Constructs a ResNet-11 model.
...
def resnet18(class_num, pretrained=False, path=None, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained.
    """
    model = ResNet(Bottleneck, [2,2,2,2], class_num, **kwargs)
    if pretrained:
        checkpoint = torch.load(path)
        state_dict = checkpoint['state_dict']
