...
def _data_transforms_tiny_imagenet():
...
def dirichlet_distribution(grouped_data, map, new_users, maximun, batch_size):
    ...
    test_data_global = test_data_global[:len(test_data_global)//15]

    return train_data_local_num_dict, train_data_local_dict,test_data_global
    

def load_tiny_imagenet(client_num_in_total, alpha, batch_size, maximun):
    class_num = 200
    n = 20
    ...
    train_transform, test_transform = _data_transforms_tiny_imagenet()

    data_dir = "/Dataset/Tiny_ImageNet/"


    trainset = datasets.ImageFolder(root=os.path.join(data_dir, 'train'), transform=train_transform)
    trainloader = torch.utils.data.DataLoader(trainset, batch_size=len(trainset),
                                            shuffle=False, num_workers=2)


    testset = datasets.ImageFolder(root=os.path.join(data_dir, 'test'),  transform=test_transform)
...
if __name__ == '__main__':
    ...
    # load_tiny_imagenet(20, 10, 32, 2000)
    print("finish")